...
lastReqTime = None
proxies = {
  'https': 'http://153.125.129.235:60088',
}
...
def _request(payloadString):
    ...
    r = scraper.get("{0}?{1}".format(baseUrl, payloadString))
...
def requestProfilePosts(memberId):
    """Method for requesting a profile posts."""
    return _request("action=profile;u={0};sa=showPosts".format(memberId))
...
=======
        ...
        "Merit:": "merit"
        "Posts:": "posts"
>>>>>>> aab8fed1349a70df81e9f6bbac579e9c91c827b7
    }
...
def parseProfilePosts(html):
    """Method for parsing profile posts HTML."""

    try:
        docRoot = lxml.html.fromstring(html)
        columns = docRoot.cssselect("#bodyarea>table td.middletext:last-child")
        label_content = columns[0].text_content()
        labels = label_content.split('on: ')
        last_time = labels[1].strip()
    except:
        last_time = None
